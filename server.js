...
app.get('/api/storage', async (req,res,next)=>{try{const rows=await sb('storage_items?select=key,value,updated_at&order=key.asc',{method:'GET'}); const data={}; const updatedAt={}; (rows||[]).forEach(r=>{data[r.key]=safeJsonParse(r.value,r.value); updatedAt[r.key]=r.updated_at;}); res.set('Cache-Control','no-store'); res.json({ok:true,data,updatedAt,database:'supabase'});}catch(e){next(e)}});
